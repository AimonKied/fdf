...
/*   Updated: 2025/06/14 23:20:41 by swied            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_minilibx(t_data *data)
{
	...
	data->mlx_window = mlx_new_window(data->mlx_connection, WINDOW_WIDTH,
			WINDOW_HEIGHT, "swied FDF");
	if (!data->mlx_window)
		return (free(data->mlx_connection), perror("MLX Window init"), -1);
	data->image = mlx_new_image(data->mlx_connection, WINDOW_WIDTH,
			WINDOW_HEIGHT);
	if (!data->image)
	{
		mlx_destroy_window(data->mlx_connection, data->mlx_window);
		free(data->mlx_connection);
		perror("MLX Image init");
		return (-1);
	}
	data->img.address = mlx_get_data_addr(data->image,
